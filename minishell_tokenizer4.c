...
/*   Updated: 2025/01/23 21:20:53 by mnaumann         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char* resolve_variables_str(char *str, t_shell *shell) {
    ...
    while (str[idx] != 0) {
        ...
        if (str[idx] == '$' && (!quote_mode || quote_mode & 2)) {
            if (str[idx + 1] == '?') {
                output_str = ft_strjoin_and_free2(output_str, get_exit_code_str(shell));
                ...
                char *var_value = get_env_var(var_name, shell->env_vars);
                if (var_value) {
                    output_str = ft_strjoin_and_free2(output_str, var_value);
                }
