...
char *handle_dollar_sign(const char *str, int *idx, char *output_str, t_shell *shell)
{
	if (str[*idx + 1] == '?')
	{
		char *exit_code_str = get_exit_code_str(shell);
	...
	{
		char *var_value = resolve_variable(str, idx, shell);
...
char *resolve_variable(const char *str, int *idx, t_shell *shell)
{
	...
	var_value = ft_getenv(shell->env_vars, var_name);
	output_str = var_value ? ft_strdup(var_value) : ft_strdup("");
	free(var_name);
	return output_str;
}
