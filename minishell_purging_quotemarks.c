...
/*   Updated: 2025/01/17 14:11:13 by mnaumann         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

//implement error handling "unclosed quotes" (in my opinion just a 
//subcase of syntax error)
void	check_for_unclosed(t_token *token_list)
{
	while (token_list->next)
		token_list = token_list->next;
	if (token_list->prev->quote_state != 0
		&& !(is_quote_char(*token_list->content)))
		return ;
}
